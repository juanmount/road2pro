...
export class SnowEngine {
  ...
  private async processTimePoint(
    ...
    if (elevationBand === 'base' && forecastHour < 24 && data.precipitation > 0.1) {
      const margin = freezingLevel - elevationMeters;
      const finalPhase = (phase.phase === 'rain' && data.temperature <= 2.5)
    ...
    return {
      ...
      windGustKmh: (() => {
        ...
        // Use original gust value from model - don't apply corrections
        // Gusts should remain as predicted by weather models
        return Math.max(Math.round(data.windGust), corrected.windSpeedCorrected);
      })(),
      ...
      // Final safety override before DB write: T<=2.5°C cannot produce pure liquid rain.
  ...
  private buildHybridForecast(multiModel: any): NormalizedForecast | null {
    ...
    // PRIORITY: Use ECMWF as primary (most accurate globally, confirmed by 14 years of research)
    // ECMWF has better spatial resolution (9km vs 13km) and more powerful infrastructure
    const primary = ecmwf || gfs;  // Prefer ECMWF (scientifically proven most accurate)
    ...
    console.log('    → Using ECMWF as primary source (most accurate model globally)');
